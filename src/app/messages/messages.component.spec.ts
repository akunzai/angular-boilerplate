...
describe('MessagesComponent', () => {
  ...
  it('Displays each message in messages', () => {
    ...
    fixture.detectChanges();
    const items: Array<HTMLDivElement> = fixture.debugElement
      // The first div is the wrapper for the test component (e.g. <div _ngcontent-a-c1>)
      .queryAll(By.css('div div p'))
      .map((e) => e.nativeElement);
    expect(items.length).toBe(2);
    expect(items[0].textContent).toContain('Alan');
    expect(items[1].textContent).toContain('Brito');
  });
});
